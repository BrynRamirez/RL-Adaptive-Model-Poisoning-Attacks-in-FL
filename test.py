...
from flwr.common import Scalar, EvaluateRes, FitRes, FitIns, parameters_to_ndarrays
...
class AggregateCustomMetricStrategy(FedAvg):
    def __init__(self, total_rounds=20, **kwargs):
    ...
    def configure_fit(self, server_round: int, parameters, client_manager):
        ...
        config = {
            ...
            "total_rounds": self.total_rounds if hasattr(self, "total_rounds") else 20,
            #"global_parameters": param_list,
        }
    ...
    def aggregate_evaluate(
        ...
        clean_avg = weighted_avg(clean_accuracies, clean_weights)
        malicious_avg = weighted_avg(malicious_accuracies, malicious_weights)
        global_avg = weighted_avg(clean_accuracies + malicious_accuracies,
                                  clean_weights + malicious_weights)

        print(f"[Round {server_round}] Global accuracy: {global_avg:.4f}")
        print(f"[Round {server_round}] Clean clients avg accuracy: {clean_avg:.4f}")
        print(f"[Round {server_round}] Malicious clients avg accuracy: {malicious_avg:.4f}")

        return global_avg, {
            "accuracy": global_avg,
            "clean_accuracy": clean_avg,
            "malicious_accuracy": malicious_avg,
        }


def start_federated_simulation(num_rounds=20, num_clients=10, dataset_type='', dataset_name='', attack_strategy=''):
...
if __name__ == "__main__":
   ...
   # MNIST Simulations
    ...
    print("Starting Adaptive Noise MNIST simulation using FedAvg.")
